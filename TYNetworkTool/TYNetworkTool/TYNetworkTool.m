        ...
        [[AFNetworkReachabilityManager sharedManager] setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
            switch (status) {
                case AFNetworkReachabilityStatusUnknown:
                    networkStatus ? networkStatus(TYNetworkStatusUnknown) : nil;
                    if (_isOpenLog) TYLog(@"未知网络");
                    break;
                case AFNetworkReachabilityStatusNotReachable:
                    networkStatus ? networkStatus(TYNetworkStatusNotReachable) : nil;
                    if (_isOpenLog) TYLog(@"无网络");
                    break;
                case AFNetworkReachabilityStatusReachableViaWWAN:
                    networkStatus ? networkStatus(TYNetworkStatusReachableViaWWAN) : nil;
                    if (_isOpenLog) TYLog(@"手机自带网络");
                    break;
                case AFNetworkReachabilityStatusReachableViaWiFi:
                    networkStatus ? networkStatus(TYNetworkStatusReachableViaWiFi) : nil;
                    if (_isOpenLog) TYLog(@"WIFI");
                    break;
            }
        }];
    });
...
+ (void)setRequestSerializer:(TYRequestSerializer)requestSerializer {
    _sessionManager.requestSerializer = requestSerializer==TYRequestSerializerHTTP ? [AFHTTPRequestSerializer serializer] : [AFJSONRequestSerializer serializer];
}

+ (void)setResponseSerializer:(TYResponseSerializer)responseSerializer {
    _sessionManager.responseSerializer = responseSerializer==TYResponseSerializerHTTP ? [AFHTTPResponseSerializer serializer] : [AFJSONResponseSerializer serializer];
}
...
@implementation NSDictionary (TY)
